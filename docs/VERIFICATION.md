...
- `npm run test:e2e`: all 4 browser journeys passed in 11.8 seconds against the real local FastAPI/SQLite app.
- Dependency installation audit: zero reported npm vulnerabilities after overriding the test-only ExcelJS UUID dependency to a patched compatible version.
